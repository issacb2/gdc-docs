...
* When experiencing download problems while using an access token try downloading a new token first before reporting it to the GDC Help Desk.  

##Logging
To provide more detailed information on an issue the GDC User Services Team may request that you run the command line application with the following flags { --debug --logfile } to assist with troubleshooting any issues that might have appeared.  These flags will run the application in debug mode and create a logfile file with the debug logs in it.  
Example Usage:
```Debug-Logfile
